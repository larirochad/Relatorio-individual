...
def gerar_csv_blocos(blocos, df_original, nome_arquivo="efeito_estrela/distancia_blocos.csv", gerar_incremento=False, nome_arquivo_incremento="efeito_estrela/distancia_blocos_incremento.csv"):
    # print(f"💾 Salvando CSV como '{nome_arquivo}'...")
    linhas = []
    linhas_incremento = []
    for i, bloco in enumerate(blocos):
        bloco = bloco.reset_index(drop=True)
        # Filtrar apenas Motion Status == 21
        ...
        for j, (_, ponto) in enumerate(bloco_21.iterrows()):
            ...
            linha_saida = {
                ...
                'GNSS UTC Time': ponto['GNSS UTC Time'],
                'Tipo Mensagem': ponto['Tipo Mensagem'],
                'Motion Status': ponto['Motion Status'],
                'Distância incremental (m)': dist_incr
            }
            linhas.append(linha_saida)
            # Para a planilha de incremento: só linhas com incremento de hodômetro
            if gerar_incremento:
    ...
    if gerar_incremento:
        ...
        print(f"✅ CSV de incremento salvo com sucesso: {nome_arquivo_incremento}")
...
def main(caminho_csv):
    ...
    if len(blocos_ignicao) == 0:
        ...
        return
    gerar_csv_blocos(blocos_ignicao, df, gerar_incremento=True)
#   print(f"\n📈 {sum(len(b) for b in blocos_ignicao)} linhas processadas. FIM.")

if __name__ == "__main__":
    main("logs/867488065171646_decoded.csv")

    # --- Plotar e salvar o gráfico da distância incremental ---
    import matplotlib.pyplot as plt
    import pandas as pd

    csv_path = "efeito_estrela/distancia_blocos.csv"
    output_img = "efeito_estrela/distancia_incremental_plot.png"
